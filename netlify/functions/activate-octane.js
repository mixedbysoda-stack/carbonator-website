...
exports.handler = async (event) => {
  ...
  const secret = process.env.OCTANE_LICENSE_SECRET;
  if (!secret) {
    console.error("OCTANE_LICENSE_SECRET not configured");
    return {
      statusCode: 500,
      headers,
  ...
  try {
    const store = getBlobStore("octane-activations");
    ...
    console.error("On Tap activation error:", err.message);
    return {
      statusCode: 500,
      headers,
