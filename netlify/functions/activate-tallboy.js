...
/**
 * TALLBOY Activation Endpoint
 * POST { key, machineID }
 * Validates HMAC-signed serial key, locks to machine (max 3), returns token.
 *
 * Mirrors activate-ontap.js exactly — same key format, same machine binding,
 * same response shape — so the plugin-side LicenseManager is shared logic.
 * Only the secret and the blob store differ.
 */
...
exports.handler = async (event) => {
  ...
  const secret = process.env.TALLBOY_LICENSE_SECRET;
  if (!secret) {
    console.error("TALLBOY_LICENSE_SECRET not configured");
    return {
      statusCode: 500,
      headers,
  ...
  try {
    const store = getBlobStore("tallboy-activations");
    ...
    if (record.machines.length >= 3) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "Maximum activations reached (3/3). Contact mixedbysoda@gmail.com for help.",
        }),
      };
    }
    ...
    console.error("TALLBOY activation error:", err.message);
    return {
      statusCode: 500,
      headers,
