...
exports.handler = async (event) => {
  ...
  const secret = process.env.ONTAP_LICENSE_SECRET;
  if (!secret) {
    console.error("ONTAP_LICENSE_SECRET not configured");
    return {
      statusCode: 500,
      headers,
  ...
  try {
    const store = getBlobStore("ontap-activations");
