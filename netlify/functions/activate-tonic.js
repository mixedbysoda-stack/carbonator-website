...
exports.handler = async (event) => {
  ...
  const secret = process.env.TONIC_LICENSE_SECRET;
  if (!secret) {
    console.error("TONIC_LICENSE_SECRET not configured");
    return {
      statusCode: 500,
      headers,
  ...
  try {
    const store = getBlobStore("tonic-activations");
    ...
    console.error("Tonic activation error:", err.message);
    return {
      statusCode: 500,
      headers,
