...
exports.handler = async () => {
  ...
  const pass = await runDripPass({
    ...
    markSent: (lead) => ({ drip_status: "email3_sent", email3_sent_at: new Date().toISOString() }),
  });
    ...
    try {
      await sendEmail(resend, {
        ...
        subject: pass.failed > 0
          ? `⚠️ Drip Day 7: ${sent} sent, ${pass.failed} failed`
        ...
        html: `<div style="font-family:Arial,sans-serif;padding:20px;background:#0d0a1a;color:#fff;">
          ...
          <p><strong>${sent}</strong> conversion email${sent === 1 ? "" : "s"} sent, ${pass.failed} failed:</p>
