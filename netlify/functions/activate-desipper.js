...
exports.handler = async (event) => {
  ...
  const secret = process.env.DESIPPER_LICENSE_SECRET;
  if (!secret) {
    console.error("DESIPPER_LICENSE_SECRET not configured");
    return {
      statusCode: 500,
      headers,
  ...
  try {
    const store = getBlobStore("desipper-activations");
    ...
    if (record.machines.length >= 3) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "Maximum activations reached (3/3). Contact support@carbonatedaudio.com for help.",
        }),
      };
    }
    ...
    console.error("De-Sipper activation error:", err.message);
    return {
      statusCode: 500,
      headers,
...
// Export for use by stripe webhook when generating keys
